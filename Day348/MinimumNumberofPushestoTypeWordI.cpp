...
// // letters. The keys can be remapped to any amount of letters, but each
// letter
...
// // key 4 "d" -> one push on key 5 "e" -> one push on key 6 Total cost is 1 +
// 1 +
...
// // Output: 12
// // Explanation: The remapped keypad given in the image provides the minimum
// // cost. "x" -> one push on key 2 "y" -> two pushes on key 2 "c" -> one push
// on
// // key 3 "d" -> two pushes on key 3 "e" -> one push on key 4 "f" -> one push
// on
// // key 5 "g" -> one push on key 6 "h" -> one push on key 7 "i" -> one push on
// // key 8 "j" -> one push on key 9 Total cost is 1 + 2 + 1 + 2 + 1 + 1 + 1 + 1
// +
// // 1 + 1 = 12. It can be shown that no other mapping can provide a lower
// cost.
