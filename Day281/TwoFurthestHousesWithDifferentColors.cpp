...
// // color 3 is green. The furthest two houses with different colors are house
// 0
// // and house 4. House 0 has color 1, and house 4 has color 3. The distance
// // between them is abs(0 - 4) = 4. Example 3:

// // Input: colors = [0,1]
// // Output: 1
// // Explanation: The furthest two houses with different colors are house 0 and
// // house 1. House 0 has color 0, and house 1 has color 1. The distance
// between
// // them is abs(0 - 1) = 1.
...
//         for (int i = 0; i < n; i++) {
//             if (colors[i] != colors[0]) { // we checked from Left to each one
//                 ans = max(ans, i);
//             }
//             if (colors[i] != colors[n - 1]) { // we checked from Right to
//             each one
//                 ans = max(ans, n-i-1);
//             }
//         }
