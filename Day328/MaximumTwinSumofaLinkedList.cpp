...
// the
// // linked list is known as the twin of the (n-1-i)th node, if 0 <= i <= (n /
// 2)
// // - 1.

// // For example, if n = 4, then node 0 is the twin of node 3, and node 1 is
// the
// // twin of node 2. These are the only nodes with twins for n = 4. The twin
// sum
...
//     stack<int> storeInStack(ListNode* head) {
//         stack<int> st;
//         while (head != nullptr) {
//             st.push(head->val);
//             head = head->next;
...
//         stack<int> st = storeInStack(head);
//         int sum = 0;
//         int half = n / 2;
//         while (half--) {
//             sum = max(sum, head->val + st.top());
//             head = head->next;
//             st.pop();
//         }
//         return sum;
